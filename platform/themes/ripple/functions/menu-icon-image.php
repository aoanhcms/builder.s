...
add_filter(BASE_FILTER_BEFORE_RENDER_FORM, function ($form, $data) {
    if (get_class($data) == MenuNode::class) {
        $iconImage = $data->icon_image ?: MetaBox::getMetaData($data, 'icon_image', true);
...
add_filter('menu_nodes_item_data', function ($data) {
    $menuNode = app(MenuNodeInterface::class)->findById($data['id']);

    if ($menuNode) {
        $data['icon_image'] = MetaBox::getMetaData($menuNode, 'icon_image', true);
    }
